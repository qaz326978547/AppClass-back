        ...
        try {
            ...
            if ($user) {
                ...
                if ($user->provider_name !== $provider) {
                    return redirect(config('app.frontend_url') . '/auth/callback' . '?provider=' . $provider . '&error=該電子郵件已通過其他提供者註冊，請使用原有提供者登入。');
                }
                // 更新提供者 ID
            ...
            $token = $user->createToken('token')->accessToken;
            // 重定向到前端页面
            return redirect(config('app.frontend_url') . '/auth/callback' . '?provider=' . $provider . '&token=' . $token);
        } catch (\Exception $e) {
            Log::error($provider . ' 登入錯誤: ' . $e->getMessage());
            return redirect(config('app.frontend_url') . '/login' . '?error=' . $e->getMessage());
        }
    }
